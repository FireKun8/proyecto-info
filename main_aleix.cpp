...
void leerDatos(mIndex mMigraciones, mGestorLoc mLocalizaciones);
...
int main(){
    ...
    leerDatos(mMigraciones, mLocalizaciones);
...
void imprimirDatos(mIndex mMigraciones, mGestorLoc mLocalizaciones, int intro){
    ...
    for(int i = 0; i < NLOC; i++){
        //separadores de continentes
        cout << "| " << setw(50) << mLocalizaciones[i].localizacion << "| " << setw(10) << mMigraciones[i].lCodigo;
        for(int j = 0; j < NANOS; j++){
            if(intro == 1){
